...
@interface LKApplePayManager ()<SKPaymentTransactionObserver, SKProductsRequestDelegate>{
...
static LKApplePayManager *_instance = nil;
@implementation LKApplePayManager


+ (instancetype)shared{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[LKApplePayManager alloc] init];
    });
...
- (void)requestProductDatasComplete:(void(^_Nullable)(NSError * _Nullable error, NSArray*_Nullable products))complete{
    [LKOrderApi fetchtAppleProductDatasComplete:^(NSError * _Nonnull error, NSArray * _Nonnull results) {
          if (error == nil) {
              NSMutableArray *productIds = [NSMutableArray array];
              for (NSDictionary *dict in results) {
                  LKGoods *goods = [[LKGoods alloc] initWithDictionary:dict];
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    ...
    if (products.count <= 0) {
        ...
        if ([product.productIdentifier isEqualToString:self.productId]) {
            ...
            NSLog(@"product.productIdentifier = %@,self.productId = %@； 不一致",product.productIdentifier,self.productId);
...
- (void)createOrderRequest:(SKProduct *)product{
    [LKOrderApi createOrderType:@"ios" withParameters:self.orderParams complete:^(NSError * _Nullable error, NSDictionary * _Nonnull result) {
...
- (void)checkPayBefore{
    ...
    LKLogInfo(@"transactions:%@",transactions);
...
- (void)removeReceipt{
    ...
    if (orderInfo != nil) {
        ...
        NSString *path = [NSString stringWithFormat:@"%@/order.plist",[LKSandBoxHelper iapReceiptPath]];
...
- (void)completeAppleTransaction:(SKPaymentTransaction *)transaction{
...
- (void)completeTransaction:(SKPaymentTransaction *)transaction{
    ...
    if (user.userId.exceptNull == nil) {
        // 如果用户信息为空延迟一秒后再进行校验
        // 日志输出
        [self logPayBehaviourReportWithProductId:self.productId WithEvent:@"USER_ID_EMPTY" WithEventName:@"用户信息还未获取到延迟1.5秒后校验" WithOrderId:@"" WithReceipt:@"" WithTransactionIdentifier:transaction.transactionIdentifier WithAllOrders:nil];
        LKLogInfo(@"用户信息还未获取到延迟1.5秒后校验");
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self completeAppleTransaction:transaction];
        });
    }else{
        LKLogInfo(@"苹果支付完成-进行后台校验");
        [self completeAppleTransaction:transaction];
    }
...
- (void)verifyReceiptWithtransaction:(SKPaymentTransaction *)transaction orderId:(NSString *)orderId receipt:(NSString *)receipt
{
    ...
    if (orderId.exceptNull == nil) {
        orderId = @"none";
        LKLogInfo(@"====丢单了===none");
        [self logPayBehaviourReportWithProductId:productId WithEvent:@"LOSE_ORDER" WithEventName:@"订单号为空" WithOrderId:orderId WithReceipt:receipt WithTransactionIdentifier:transactionIdentifier WithAllOrders:[self getReceiptFromeFile]];
    }
    if (receipt.exceptNull != nil) {
        
        [LKOrderApi appleFinishOrderNum:orderId receipt:receipt transactionIdentifier:transactionIdentifier subscribe:NO complete:^(NSError * _Nullable error, NSDictionary * _Nullable result) {
...
- (void)logPayBehaviourReportWithProductId:(NSString *)productId WithEvent:(NSString *)event WithEventName:(NSString *)eventName WithOrderId:(NSString *)orderId WithReceipt:(NSString *)receipt WithTransactionIdentifier:(NSString *)transactionIdentifier WithAllOrders:(NSDictionary *)orderInfos{
    ...
    [LKPointApi logReportServerWithEvent:event eventName:eventName infos:result WithOtherLogInfo:orderInfos complete:^(NSError * _Nullable error) {
...
- (void)logPointPayWithorderId:(NSString *)orderId productId:(NSString *)productId{
    ...
    if (price != nil && orderId != nil) {
        ...
        LKLogInfo(@"AF pay Point Success");
    }
