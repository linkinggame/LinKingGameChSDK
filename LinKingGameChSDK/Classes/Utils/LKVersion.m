...
#import "LKLog.h"
@interface LKVersion ()<SKStoreProductViewControllerDelegate>
...
static LKVersion *_instance = nil;

@implementation LKVersion


+ (instancetype)shared{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[LKVersion alloc] init];
...
//            }];
            UIAlertAction *confimAction = [UIAlertAction actionWithTitle:self.confimTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                if (self.goUpdateCallback) {
                    self.goUpdateCallback();
                }
            }];
...
//            [alertControler addAction:cancelAction];
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:self.alertTitle message:updateMessage delegate:self cancelButtonTitle:self.confimTitle otherButtonTitles:nil];
            [alertView show];
        }
    }
}
